...
$(document).ready(function () {
    ...
    function check_on_fs_gallery_only_one_type_element(what_insert) {
        var gallery_isset_video = false;
        var gallery_isset_image = false;
        if (jQuery('#page_template').val() == "page-fullscreen-gallery.php") {
            /*Check if video isset*/
            jQuery('.selected_media ul li').each(function (index) {
                if (jQuery(this).find(".item-with-settings").hasClass("video_slide")) {
                    gallery_isset_video = true;
                }
                if (jQuery(this).find(".item-with-settings").not(".video_slide")) {
                    gallery_isset_image = true;
                }
            });
        }
        if (what_insert == "video" && gallery_isset_video == true) {
            alert("Note: Unfortunately you can add only 1 video!");
            return false;
        }
        if (what_insert == "image" && gallery_isset_video == true) {
            alert("Note: Sorry, but you have 1 video added. You can add either images or 1 video to the fullscreen gallery!");
            return false;
        }
        if (what_insert == "video" && gallery_isset_image == true) {
            alert("Note: Sorry, but you have images added. You can add either images or 1 video to the fullscreen gallery!");
            return false;
        }
    }
    ...
    $(document).on('click', '.slider_type .available_media_item', function () {
        if (check_on_fs_gallery_only_one_type_element("image") == false) {return false;}
    ...
    $(document).on('click', '.slider_type .add_video_slider', function () {
        if (check_on_fs_gallery_only_one_type_element("video") == false) {return false;}
        if (jQuery('#page_template').val() == "page-gallery-kenburns.php" || jQuery('#page_template').val() == "page-gallery-grid.php" || jQuery('#page_template').val() == "page-gallery-masonry.php") {
            alert("You can use images only for this type of gallery.");
        } else {
            var available_media_item_this_url = $(this).find('.previmg').attr('data-full-url');
            var parent_root = $(this).parents('.bg_or_slider_option');
            var settings_type = $(this).parents('.bg_or_slider_option').find('.settings_type').val();
