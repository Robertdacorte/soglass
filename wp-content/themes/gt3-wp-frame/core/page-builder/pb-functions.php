...
function get_selected_pf_images_for_admin($pagebuilder)
{
    ...
    if (isset($pagebuilder['post-formats']['images']) && is_array($pagebuilder['post-formats']['images'])) {
        foreach ($pagebuilder['post-formats']['images'] as $imgid => $img) {
            $compile .= "<div class='img-item style_small'><div class='img-preview'><img src='".aq_resize($img['src'], "62", "62", true, true, true)."' data-full-url='{$img['src']}' data-timthumb-url='".aq_resize($img['src'], "156", "106", true, true, true)."' alt='' class='previmg'><div class='hover-container'></div><div class='deldel-container'></div></div><input type='hidden' name='pagebuilder[post-formats][images][" . $imgid . "][src]' value='{$img['src']}'></div>";
        }
    }
    return $compile;
}
        ...
        foreach ($array as $key => $slide) {
            ...
            if ($slider_type == "fullscreen") {
                ...
                if ($slide['slide_type'] == "video") {
                    $compile .= "
                    <div class='img-item item-with-settings video_slide'>
                        <input type='hidden' name='pagebuilder[sliders][fullscreen][slides][{$key}][src]' value='{$slide['src']}'>
                    ...
                    /*echo "            <div class='padding-cont' style='padding-top:0;'>
